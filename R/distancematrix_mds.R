...
#'
#' \code{distanceMatrix()} creates a distance matrix from a list of MS2 spectra,
#' MS1 pseudospectra or neutral loss patterns by pairwise comparison using the
#' specified distance function. This distance matrix is the basis for CluMSID's
...
#'   \code{speclist}. Row and column names are taken from the \code{id} slot or,
#'   if present, pasted from the \code{id} and \code{annotation} slots of the
...
distanceMatrix <- function(speclist, distFun = "cossim", type = "spectrum"){
  if(distFun == "cossim"){
    ...
    for (m in 1:length(speclist)) {
      for (l in m:length(speclist)) {
        if (is.na(distmat[m, l])) {
          distmat[m, l] <- 1 - cossim(speclist[[m]], speclist[[l]], type = type)
  ...
  for(e in 1:length(speclist)){
    if(S4Vectors::isEmpty(speclist[[e]]@annotation) || speclist[[e]]@annotation == ""){
      featnames[e] <- speclist[[e]]@id
    } else {
      featnames[e] <- paste(speclist[[e]]@id, speclist[[e]]@annotation, sep = " - ")
    }
  }
...
#'   points are grey in the MDS plot.
#'
#' @return An MDS plot generated with the help of \code{\link[stats]{cmdscale}},
#'   \code{\link[ggplot2]{ggplot}} and, if interactive,
#'   \code{\link[plotly]{ggplotly}}.
#'
#' @export
CluMSID_MDS <- function(distmat, interactive = FALSE, highlight_annotated = FALSE){
  ...
  if(highlight_annotated == TRUE){
    q <- ggplot2::ggplot(fit, ggplot2::aes(x = fitx, y = fity, text = anno)) +
      ggplot2::geom_point(pch = 16, alpha = 0.5, size = 2,
                          colour = as.numeric(grepl(pattern = " - ", x = fit$anno))+1) +
