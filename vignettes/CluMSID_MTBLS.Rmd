...
mtbls_df <- readr::read_delim(raw_mtbls_df, "\t") %>%
    dplyr::mutate(metabolite_identification = 
               stringr::str_replace(metabolite_identification, "unknown", "")) %>%
...
```{r dendro, fig.width=12, fig.asp=1.2, fig.cap=fig1}
CluMSID_HCplot(distmat, h = 0.7, cex = 1)
```
