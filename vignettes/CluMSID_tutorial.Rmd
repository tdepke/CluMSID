...
load(file = system.file("extdata", "distmat.RData", package = "CluMSID"))
load(file = system.file("extdata", "nlmat.RData", package = "CluMSID"))
```
...
For MS^2^ spectra:
```{r CluMSID_MDSplot_1, fig.width=7, fig.asp=1, fig.cap=fig1}
CluMSID_MDS(distmat, highlight_annotated = TRUE)
```

For neutral loss patterns:
```{r CluMSID_MDSplot_2, fig.width=7, fig.asp=1, fig.cap=fig2}
...
```{r CluMSID_HCplot_3, echo=FALSE, out.width="100%", fig.cap=fig8}
knitr::include_graphics(system.file("extdata", 
                                    "CluMSID_dendro2.png", 
                                    package = "CluMSID"))
```
...
```{r Pseudospectra_4, , fig.width=7, fig.asp=1, fig.cap=fig12}
